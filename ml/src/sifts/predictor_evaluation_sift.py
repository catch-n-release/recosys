...
from ml.utils import boot_config, sift_success, DeepchecksModelWrapper, boot_dc_dataset, boot_train_test
from ml import log
...
def boot_predictor(config: type) -> AlgoBase:
    try:
        return dump.load(config.ml.path.predictor)[1]
...
def predictor_eval_sift_suite(
    ...
    try:
        ...
        if ignore_ticks := list(config.ml.data.ticks.ignore_pe):
            for tick in ignore_ticks:
                pes_suite.remove(tick)
        result = pes_suite.run(
            train_dataset=datasets.get(config.ml.const.trainset),
            test_dataset=datasets.get(config.ml.const.trainset),
            model=DeepchecksModelWrapper(predictor),
        )
        result.save_as_html(config.ml.path.algo_eval_sift_html)
        return result
