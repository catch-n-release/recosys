...
def boot_processed_dataframe(config: type) -> pd.DataFrame:
    try:
        return pd.read_csv(config.ml.path.processed_dataset)
...
def save_splits(
    ...
    try:
        datasets.get(config.ml.const.trainset).data.to_csv(
            config.ml.path.trainset)
        datasets.get(config.ml.const.testset).data.to_csv(
            config.ml.path.testset)
    except Exception as e:
        raise e
...
def train_test_sift_suite(
    ...
    try:
        ...
        if ignore_ticks := list(config.ml.data.ticks.ignore_ttv):
        ...
        result.save_as_html(config.ml.path.tt_sift_html)
        return result
    except Exception as e:
        raise e
