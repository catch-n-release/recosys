...
from utils import boot_train_test, boot_config
from ml import log
...
def surprise_dataset(
    ...
    try:
        ...
        dataset = Dataset.load_from_df(
            dataset[list(config.app.ml.data.columns)],
            Reader(rating_scale=tuple(config.app.ml.data.rating_scale)),
        )
        return dataset
...
def train():
    try:
        ...
        trainset = surprise_dataset(
            config,
            datasets.get(config.app.ml.const.trainset),
        )
        testset = surprise_dataset(
            config,
            datasets.get(config.app.ml.const.testset),
        )
